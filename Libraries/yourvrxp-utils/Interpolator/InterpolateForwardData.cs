...
namespace yourvrexperience.Utils
{
    public class InterpolateForwardData : IEquatable<InterpolatePositionData>, IInterpolateData
	{
        ...
        public InterpolateForwardData(GameObject actor, Vector3 origin, Vector3 goal, float totalTime, float timeDone, bool setTargetWhenFinished)
		{
			_gameActor = actor;
            _activated = true;
		...
		{
			_origin = new Vector3(origin.forward.x, origin.forward.y, origin.forward.z);
		...
		public bool Inperpolate()
		{
            ...
            if (_timeDone <= _totalTime)
			{
				...
				_gameActor.transform.forward = _origin + (increaseFactor * eulerTarget);
				return false;
			}
			else
			{
