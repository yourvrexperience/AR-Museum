...
namespace yourvrexperience.Utils
{
    ...
    public class InterpolateScaleData : IEquatable<InterpolatePositionData>, IInterpolateData
    {
        ...
        public int TypeData
        {
            get { return InterpolatorController.TypeInterpolateScale; }
        }

        public InterpolateScaleData(GameObject actor, Vector3 origin, Vector3 goal, float totalTime, float timeDone, bool setTargetWhenFinished)
		{
			_gameActor = actor;
            _activated = true;
		...
		{
			_origin = new Vector3(origin.localScale.x, origin.localScale.y, origin.localScale.z);
		...
		public bool Inperpolate()
		{
            ...
            if (_timeDone <= _totalTime)
			{
				...
				_gameActor.transform.localScale = _origin + (increaseFactor * forwardTarget);
				return false;
			}
			else
			{
